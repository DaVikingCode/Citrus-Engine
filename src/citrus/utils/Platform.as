package citrus.utils {
	...
	/**
	 * updated: March 27th 2015 by @SnkyGames
	 * updated: November 09th 2015 by @SnkyGames
	 * 
	...
	final public class Platform {
		...
		static private var _PLATFORM_SPECIFICS_IS_LANDSCAPE : Boolean;
		...
		// /...
		// /PRIVATE FINISHED FUNCTIONS - unused publically, only ever called once, then caches results in the static vars.
		...
		static private function queryIphoneFour() : uint {
			if (queryIOS()) {
				stageValidate();
				if (isLandscape())
					return ( uint(( _STAGE.fullScreenWidth == _IPHONE_RETINA_FOUR_HEIGHT ) && (_STAGE.fullScreenHeight == _IPHONE_RETINA_FOUR_WIDTH )) || uint(( _STAGE.fullScreenWidth == ( _IPHONE_RETINA_FOUR_HEIGHT - _IOS_RETINA_STATUSBAR_HEIGHT ) && ( _STAGE.fullScreenHeight == ( _IPHONE_RETINA_FOUR_WIDTH - _IOS_RETINA_STATUSBAR_HEIGHT )) )) );
				else
					return ( uint(( _STAGE.fullScreenWidth == _IPHONE_RETINA_FOUR_WIDTH ) && (_STAGE.fullScreenHeight == _IPHONE_RETINA_FOUR_HEIGHT )) || uint(( _STAGE.fullScreenWidth == ( _IPHONE_RETINA_FOUR_WIDTH - _IOS_RETINA_STATUSBAR_HEIGHT ) && ( _STAGE.fullScreenHeight == ( _IPHONE_RETINA_FOUR_HEIGHT - _IOS_RETINA_STATUSBAR_HEIGHT )) )) );
			} else
				return 0;
		}
		...
		static private function queryIphoneFive() : uint {
			if (queryIOS()) {
				stageValidate();
				if (isLandscape())
					return ( uint(( _STAGE.fullScreenWidth == _IPHONE_RETINA_FIVE_HEIGHT ) && (_STAGE.fullScreenHeight == _IPHONE_RETINA_FIVE_WIDTH )) || uint(( _STAGE.fullScreenWidth == ( _IPHONE_RETINA_FIVE_HEIGHT - _IOS_RETINA_STATUSBAR_HEIGHT ) && ( _STAGE.fullScreenHeight == ( _IPHONE_RETINA_FIVE_WIDTH - _IOS_RETINA_STATUSBAR_HEIGHT )) )) );
				else
					return ( uint(( _STAGE.fullScreenWidth == _IPHONE_RETINA_FIVE_WIDTH ) && (_STAGE.fullScreenHeight == _IPHONE_RETINA_FIVE_HEIGHT )) || uint(( _STAGE.fullScreenWidth == ( _IPHONE_RETINA_FIVE_WIDTH - _IOS_RETINA_STATUSBAR_HEIGHT ) && ( _STAGE.fullScreenHeight == ( _IPHONE_RETINA_FIVE_HEIGHT - _IOS_RETINA_STATUSBAR_HEIGHT )) )) );
			} else
				return 0;
		}
		...
		static private function queryIpadRetinaLegacy() : uint {
			if (queryIOS()) {
				if (isLandscape())
					return ( uint(_STAGE.fullScreenWidth == _IPAD_RETINA_LEGACY_HEIGHT) || uint(_STAGE.fullScreenWidth == ( _IPAD_RETINA_LEGACY_HEIGHT - _IOS_RETINA_STATUSBAR_HEIGHT )) );
				else
					return ( uint(_STAGE.fullScreenWidth == _IPAD_RETINA_LEGACY_WIDTH) || uint(_STAGE.fullScreenWidth == ( _IPAD_RETINA_LEGACY_WIDTH - _IOS_RETINA_STATUSBAR_HEIGHT )) );
			} else
				return 0;
		}
		...
		static private function queryIpadRetinaPro() : uint {
			if (queryIOS()) {
				if (isLandscape())
					return ( uint(_STAGE.fullScreenWidth == _IPAD_RETINA_PRO_HEIGHT) || uint(_STAGE.fullScreenWidth == ( _IPAD_RETINA_PRO_HEIGHT - _IOS_RETINA_STATUSBAR_HEIGHT )) );
				else
					return ( uint(_STAGE.fullScreenWidth == _IPAD_RETINA_PRO_WIDTH) || uint(_STAGE.fullScreenWidth == ( _IPAD_RETINA_PRO_WIDTH - _IOS_RETINA_STATUSBAR_HEIGHT )) );
			} else
				return 0;
		}

		// It is indeed possible to get a more specific browser.. ( I don't think that it spits out an accurate result though )
		...
		// /Could potentially cache all device specifications in one call, broke the first time I tested that
		static private function queryIOS() : uint {
			_PLATFORM_IS_IOS = uint(_PLAYER_VERSION == "IOS");
			if (1 == _PLATFORM_IS_IOS) {
				// ...query everything iOS
			}
		...
		// TODO: If the user disables statusbar, set statusbar height to 0.0f
		// TODO: Display Zoom check + new zoom width / height values
